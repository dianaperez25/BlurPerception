...
for s = [7 10 18 25 26 29 30 33 39 41 44 45 48 52 53 56 57 60 61 64]
    ...
    filename = strcat('V9B_s',num2str(s),'_stim',num2str(stim));
    savefile = strcat('V9B_s',num2str(s),'_stim',num2str(stim),'_Results');
            
    Acc = load('AccuraciesV9B',filename);
    Acc = struct2cell(Acc);
    
    [fitresult, gofStim] = createFitV9B(StimLevels, Acc{1,1}, filename);
...
save('V9BFittedResults', 'Results')

xlswrite('V9BFittedResults',Results);
