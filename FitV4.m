...
for s = 1:24
    ...
    filename = strcat('V4_s',num2str(s),'_stim',num2str(stim),'prime',num2str(prime));
    savefile = strcat('V4_s',num2str(s),'_stim',num2str(stim),'prime',num2str(prime),'_Results');
            
    Acc = load('AccuraciesV4',filename);
    Acc = struct2cell(Acc);
    
    [fitresult, gofStim] = createFitV4(StimLevels, Acc{1,1}, filename);
...
save('V4FittedResults', 'Results')

xlswrite('V4FittedResults',Results);
